...
describe('debounce', () => {
	...
	it('checks the state content', async () => {
		const debounced = debounce(() => undefined, 10);
		expect(debounced.state$.content()).to.eq('idle');
		debounced();
		expect(debounced.state$.content()).to.eq('debouncing');
		await new Promise<void>((res) => setTimeout(res, 11));
		expect(debounced.state$.content()).to.eq('idle');
	});
	it('checks the state content with requestAnimationFrame', async () => {
